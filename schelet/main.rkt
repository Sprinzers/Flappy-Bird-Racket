...
(define-struct birdstruct (posx posy velocity))

(define b1 (birdstruct bird-x bird-initial-y 0))

(define-struct statestruct (birdstruct))
...
(define (get-initial-state)
  (birdstruct bird-x bird-initial-y 0))
...
(define (get-bird-y bird)
  (birdstruct-posy bird))
(define (get-bird-x bird)
  (birdstruct-posx bird))
...
(define (next-state-bird bird gravity)
  (struct-copy birdstruct bird [posx (get-bird-x bird)]
               [posy (+ (get-bird-y bird) (get-bird-v-y bird))]
               [velocity (+ (get-bird-v-y bird) gravity)]))

;TODO 4
; După aceasta, implementati un getter care extrage din structura voastră
; viteza pe y a păsării.
(define (get-bird-v-y bird)
  (birdstruct-velocity bird))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy birdstruct bird [posx (birdstruct-posx bird)]
               [posy (birdstruct-velocity bird)]
               [velocity (- 0 momentum)]))
...
(define (change current-state pressed-key)
  (if (equal? pressed-key " ")
      ...
      (next-state-bird-onspace (get-bird current-state) 0)))
...
(define (next-state current-state)
  (next-state-bird  (get-bird current-state) initial-gravity))
